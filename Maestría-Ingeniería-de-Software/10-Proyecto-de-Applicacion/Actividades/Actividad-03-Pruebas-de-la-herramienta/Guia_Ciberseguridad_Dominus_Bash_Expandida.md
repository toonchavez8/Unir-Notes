...
La seguridad no debe comenzar con un escáner. Primero hay que demostrar que el código evaluado compila, que sus pruebas normales pasan y que sabemos exactamente qué versión estamos ejecutando.
...
> Si se utiliza WSL en lugar de Git Bash, las rutas pueden necesitar el formato `/mnt/c/Users/...`.
...
Si alguno falla, **detener la actividad**. No continuar cambiando rutas manualmente dentro de los comandos posteriores sin registrar la modificación.
...
### 7.3. Ejecutar Las Pruebas Normales Del Broker
...
No asumir que `5000` y `8000` son correctos si la configuración del proyecto utiliza otros puertos. Confirmarlos en la configuración del broker.

### 9.3. Iniciar El Laboratorio

Este repositorio no incluye un `docker compose` de laboratorio para `dominus-broker`. Para esta guía se utiliza la imagen Docker construida previamente:

```bash
docker image ls dominus-broker:security-lab
```

Si el laboratorio ya se ejecutó antes, limpiar los contenedores anteriores antes de repetir la prueba:

```bash
docker rm -f dominus-broker-lab redis 2>/dev/null || true
```

Crear una red local para que el broker pueda resolver Redis con el hostname `redis`:

```bash
docker network create dominus-lab 2>/dev/null || true
```

Iniciar Redis. El `env/env.prod.json` del broker apunta a `redis:6379` y usa `username=dominus` / `password=dominus`, por eso el contenedor debe llamarse `redis` dentro de la red del laboratorio:
...
Crear el usuario ACL que espera el broker:

```bash
docker exec redis redis-cli ACL SETUSER dominus on '>dominus' allcommands allkeys
docker exec redis redis-cli --user dominus -a dominus ping
```

Iniciar el broker con los puertos documentados por el repositorio:

```bash
docker run -d \
  --name dominus-broker-lab \
  --network dominus-lab \
  -p 8000:8000 \
  -p 5000:5000 \
  dominus-broker:security-lab
```
...
Guardar evidencia de arranque:

```bash
mkdir -p security-evidence
docker logs --tail=100 dominus-broker-lab 2>&1 | tee security-evidence/E21-broker-startup.log
```
...
```bash
curl -i \
  -H 'x-api-key: dominus_example_@10102024KeyServerToken' \
...
Esta prueba requiere una prueba automatizada porque enviar manualmente diez peticiones no garantiza que lleguen simultáneamente.
...
Si el protocolo requiere gRPC real, utilizar un servidor gRPC de prueba en lugar de `nc`.
...
Si el error se descarta y se envía un payload vacío, documentarlo como pérdida potencial de integridad.
...
No corregir varios hallazgos simultáneamente. La trazabilidad debe ser:
...
Esta búsqueda es una revisión auxiliar. También inspeccionar manualmente capturas.
...
```text
01. Abrir Bash
02. Definir brokerPath y sdkPath
03. Verificar directorios
04. Registrar rama y commit
05. Registrar cambios locales
06. Registrar versiones de Go/Docker
07. Descargar dependencias
08. Ejecutar go test
09. Ejecutar go test -race
...
Si se utilizó el laboratorio Docker de `dominus-broker`:

```bash
docker rm -f dominus-broker-lab redis
docker network rm dominus-lab 2>/dev/null || true
```
